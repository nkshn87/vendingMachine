...
var CreateOutputText = /** @class */ (function () {
    ...
    CreateOutputText.prototype.addText = function (text) {
        ...
        if (this.outputTextList.length)
            this.outputTextList.push('');
        this.outputTextList = this.outputTextList.concat(text.split('\n'));
    };
    // アウトプット用テキストを返却
    CreateOutputText.prototype.outputText = function (text) {
        this.addText(text);
        var returntext = this.wrap(this.outputTextList);
        this.outputTextList = [];
        return returntext;
    };
    // 蓄積したテキストリストを結合・装飾した文字列を返却
    CreateOutputText.prototype.wrap = function (textList) {
        var e_1, _a, e_2, _b;
        var returnText = '';
        var maxLength = 0;
        var textLength = 0;
...
module.exports = new CreateOutputText();
